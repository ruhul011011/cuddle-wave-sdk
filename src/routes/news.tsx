            ...
            {items.map((a: any) => (
              ...
              >
                <div className="aspect-[16/9] overflow-hidden bg-secondary">
                  {a.cover_image ? (
                    <img src={a.cover_image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" />
                  ) : (
                    <div className="h-full w-full bg-gradient-to-br from-primary/30 to-primary/10" />
                  )}
                </div>
                <div className="p-4">
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-primary">{a.category}</div>
                  <h2 className="mt-1 line-clamp-2 font-display text-lg font-semibold leading-snug">{a.title}</h2>
                  {a.excerpt && <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{a.excerpt}</p>}
                  <div className="mt-3 text-xs text-muted-foreground">
                    {(a.published_at ? new Date(a.published_at) : new Date(a.created_at)).toISOString().slice(0, 10)}
                  </div>
                </div>
              </Link>
            ))}
