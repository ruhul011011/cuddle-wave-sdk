...
ALTER TABLE public.match_access ADD CONSTRAINT match_access_access_check
  CHECK (access = ANY (ARRAY['free'::text, 'premium'::text, 'ads'::text, 'mix'::text, 'paid'::text, 'preview'::text]));
...
BEGIN
  ...
  -- 'preview' returns streams to any signed-in user; the 2-minute paywall is enforced in the player.
